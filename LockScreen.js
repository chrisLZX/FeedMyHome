var iniFeed = idTitleRss+1;var nocity1=0;var nocity2=0;var timer1;var timer2;var xmlFeed = new XMLHttpRequest();var postal;var demoMode = false;var enabled;var xmlReq = new XMLHttpRequest();var launchReq = new XMLHttpRequest();var secleft=0;var newleft=0;
...
function showElements(){
	if(xmlReq.responseXML!=null){
		var nsms=xmlReq.responseXML.getElementsByTagName("SMS")[0].firstChild.nodeValue;
		var ncall=xmlReq.responseXML.getElementsByTagName("Call")[0].firstChild.nodeValue;
		var nmail=xmlReq.responseXML.getElementsByTagName("Mail")[0].firstChild.nodeValue;
		if(nsms>0 && wannaBlink==1) document.getElementById("SMSIcon").innerHTML = "<img src='imgs/sms.gif' />" + nsms +" "+ Msms;
		else document.getElementById("SMSIcon").innerHTML = "<img src='imgs/sms.png' />" + nsms +" "+ Msms;
		if(ncall>0 && wannaBlink==1) document.getElementById("CallIcon").innerHTML = "<img src='imgs/call.gif' />" + ncall +" "+ Mcall;
		else document.getElementById("CallIcon").innerHTML = "<img src='imgs/call.png' />" + ncall +" "+ Mcall;
		if(nmail>0 && wannaBlink==1) document.getElementById("MailIcon").innerHTML = "<img src='imgs/mail.gif' />" + nmail +" "+Mmail;
		else document.getElementById("MailIcon").innerHTML = "<img src='imgs/mail.png' />" + nmail +" "+Mmail;
	}else setTimeout('showElements()',300);
}
...
function colocaFeed(){secleft+=1.4;var postersDiv = document.getElementById("feedRss");if(secleft<=10){postersDiv.style.opacity = secleft/10;setTimeout('colocaFeed()',30);}else secleft=0;}

function moverFeed(){var postersDiv = document.getElementById("feedRss");if(newleft>-280){newleft -= 70;postersDiv.style.left = newleft;setTimeout('moverFeed()',40);	}else{postersDiv.style.display='none';postersDiv.style.left=7+'px';getFeed();postersDiv.style.display='inline';colocaFeed();newleft=0;}}
