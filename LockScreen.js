var iniFeed = idTitleRss+1;var nocity1=0;var nocity2=0;var timer1;var timer2;var xmlFeed = new XMLHttpRequest();var postal;var demoMode = false;var enabled;var xmlReq = new XMLHttpRequest();var launchReq = new XMLHttpRequest();
...
function showElements(){if(xmlReq.responseXML!=null){document.getElementById("SMSIcon").innerHTML = "<img src='imgs/sms.png' />" + xmlReq.responseXML.getElementsByTagName("SMS")[0].firstChild.nodeValue +" "+ Msms;document.getElementById("CallIcon").innerHTML = "<img src='imgs/call.png' />" + xmlReq.responseXML.getElementsByTagName("Call")[0].firstChild.nodeValue +" "+ Mcall;document.getElementById("MailIcon").innerHTML = "<img src='imgs/mail.png' />" + xmlReq.responseXML.getElementsByTagName("Mail")[0].firstChild.nodeValue +" "+Mmail;}else setTimeout('showElements()',300);}
...
function colocaFeed(){
secleft+=1.4;
var postersDiv = document.getElementById("feedRss");
if(secleft<=10){
	postersDiv.style.opacity = secleft/10;
	setTimeout('colocaFeed()',30);
}else secleft=0;
...
if(newleft>-280){
	newleft -= 70;
	postersDiv.style.left = newleft;
	setTimeout('moverFeed()',40);
}else{
	postersDiv.style.display='none';
	postersDiv.style.left=7+'px';
	getFeed();
	postersDiv.style.display='inline';
	colocaFeed();
	newleft=0;
}
}
...
function processXml2(){
	if (xmlFeed.readyState != 4)	return;
	if (xmlFeed.status != 200 && xmlFeed.status != 0)return;
	else getFeed();
}
